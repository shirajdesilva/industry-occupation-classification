...
It programmatically parses fragmented state source files into a clean, queryable **Parquet** data lake. Extracting unstructured data from PDFs and Word docs into a clean, columnar format provides a robust foundation for all downstream logic.
...
The pipeline codifies state-specific rules directly into the flow. While embeddings handle the baseline matching, it leverages (planned) **LLMs** to interpret nuanced business rules (e.g., physical location exceptions for clerical staff) that traditional semantic search might miss. (Work in progress)
...
|---|---|---|
| **Embeddings** | `sentence-transformers` | Fast on CPU, semantic similarity. |
| **Data Processing** | `pandas` | High-performance tabular data manipulation. |
...
> **Caution:** This pipeline is a **proof of concept** and must not be relied upon for production, commercial classification decisions. Because these classifications directly impact financial premiums and regulatory compliance, outputs should always be reviewed by a qualified professional. The pipeline is designed for a **human-in-the-loop** auditing mechanism to spot-check logic and catch edge cases — no automated output should be treated as authoritative without independent verification.
